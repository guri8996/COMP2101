...
#         host names, etc.
########### TASK 1 #########         cleaning up the mess by putting command output in variable
hostname=$(hostname)
...
########## TASK 2 ################      printing routerAddress and routerHostname using ip route  and getent hosts and cutting out the output wee needed using awk
interfacename=$(ip a | awk '/7:/ {print $2}')
routerAddress=$(ip r | awk '/default via/ {print $3}')
